...
- **macOS:** staged writes always; Seatbelt session-attach on product majors 14–26 (capability gate) via the same `--os-sandbox` flag. Seatbelt attach evidence is local; CI attach evidence is linux amd64 (Landlock).
