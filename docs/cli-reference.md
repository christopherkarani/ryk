...
doctor reports readiness and host capabilities. doctor --check is the automation gate and returns nonzero when core readiness fails. doctor --json is a small readiness document, but a JSON result with ready: false can still exit zero. Use --check when the process exit status is the gate. doctor --fix is a mutating repair path and cannot be combined with --check or --json. Diagnose names Pi/Grok hooks that point at a Zig test program (wired: broken). doctor --fix, run as product ryk from a normal terminal, rebinds those hooks to the running ryk binary. A --fix invoked as zig-cache `test` refuses to rewrite hooks and says so.
...
--verify checks the session hash chain. `ryk run` session end prints `Audit chain: <sha256>` (same value as `final_event_hash` in the summary). `ryk mcp proxy` prints `ryk mcp proxy: audit chain <sha256>` on stderr. The printed hash is a copy-out-of-band convenience, not an integrity guarantee. A local rewrite of both the event log and the summary is out of scope. report produces a summary and export:
