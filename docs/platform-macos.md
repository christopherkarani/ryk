...
- **Ancestor instruction RO:** existing `AGENTS.md` / `AGENTS.MD` / `CLAUDE.md` / `CLAUDE.MD` files on ancestors of the workspace (from the workspace parent up through `$HOME`) that the host-runtime catalog calls **instruction** are granted as **file-only RO** so agents that walk up for instruction files (pi, Claude, …) do not EPERM. Skill trees are not granted here. Never a parent-dir tree grant, never bare `$HOME`, and never a file above `$HOME`.
