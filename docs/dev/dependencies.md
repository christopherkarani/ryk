...
- HTTP/TLS (`std.http.Client`) lives in `telemetry_transport.zig` and `intercept/provider_gateway.zig`. Default `zig build` / PATH / curl|sh keeps both (`-Dhttp` defaults true; live releases set a PostHog token). `-Dhttp=false` omits those modules so TLS is not linked. Shared types live in `provider_gateway_types.zig` (no `std.http`) so the slim stub cannot copy `Limits` / audit shapes. `./scripts/zig build check-http-slim` runs the stub fail-closed tests; `./scripts/zig build -Dhttp=false check` compiles the full CLI against those stubs. Empty-token dry-run does not shrink the token-present release binary. Isolation of the two constructors alone is a size no-op.
- **One shipped `bin/ryk`.** Do not add an optional `ryk-dev` sibling. Use `-Dtui` / `-Dhttp` for slim compile-outs. PATH `ryk` must keep doctor / update / uninstall / hook / evaluate / run / host aliases / `agent_hook`. A second binary does not shrink curl|sh, Homebrew, npm, or checksums. Size snapshots live in [`binary-size-baseline.tsv`](binary-size-baseline.tsv); PCRE2 slim notes are in [`pcre2-slim.md`](pcre2-slim.md). P4 did not land a second shipped binary.
