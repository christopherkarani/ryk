...
- HTTP/TLS (`std.http.Client`) lives in `telemetry_transport.zig` and `intercept/provider_gateway.zig`. Default `zig build` / PATH / curl|sh keeps both (`-Dhttp` defaults true; live releases set a PostHog token). `-Dhttp=false` omits those modules so TLS is not linked. Empty-token dry-run does not shrink the token-present release binary. Isolation of the two constructors alone is a size no-op.
