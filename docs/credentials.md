...
| **Azure SAS** | query or connection string contains both `sv=` and `sig=` at `?` / `&` / `;` / `=` / `&amp;` boundaries; signature ≥ 16 chars and ≥ 8 distinct (restricted alphabet) | `https://example.invalid/blob?sv=2021-06-08&sig=fakeSyntheticAzureSasSigValue` |
