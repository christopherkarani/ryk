...
- `test-openclaw-release-assets.sh` proves committed OpenClaw `dist/` matches TypeScript. `agent-gate` runs it when `integrations/openclaw-plugin/**` is dirty (plugin gate); release packaging also runs it. GitHub Actions `test.yml` job `openclaw-release-assets` runs it after `npm ci` with `RYK_RELEASE_LIVE=1` when the plugin tree or this script changes; a missing `tsc` is a hard fail on that CI path (`GITHUB_ACTIONS` is also fail-closed). Local `agent-gate` / `verify-pre-merge` still skip freshness when plugin `node_modules` are not installed.
