...
- `test-openclaw-release-assets.sh` proves committed OpenClaw `dist/` matches TypeScript. `agent-gate` runs it when `integrations/openclaw-plugin/**` is dirty (plugin gate) and skips freshness when plugin `node_modules` are not installed. Live release (`release.yml`, `RYK_RELEASE_LIVE=1`) installs plugin deps and fail-closes on a missing `tsc` or stale `dist/`. PR `test.yml` does not run this check.
