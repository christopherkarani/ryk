...
assert_absent scripts/install.sh 'print_activation|eval "\$\('
assert_absent scripts/install.ps1 'Write-Activation|then evaluate the set commands'
assert_absent src/cli/update.zig 'ryk start|eval "\$\(|doctor --fix --from-install'
