#!/usr/bin/env bash
# Print incoming command + host-shaped allow/deny for every ryk envelope
...
#
# Exit 0 only when every envelope allows `git status` and blocks `rm -rf /`.
  ...
  if [[ "$expect" == "allow" && "$decision" == "allow" && "$code" == "0" ]]; then
    ...
    case "$host" in
      codex)
        ...
        ;;
      grok)
        # Official Grok contract is exit 2 + decision JSON. Empty stdout is not enough.
        if [[ "$code" == "2" && "$decision" == "block" ]]; then ok=1; fi
        ;;
      pi)
        # Pi evaluate deny requires decision JSON. Exit 2 with empty stdout is not a pass.
        if [[ "$decision" == "block" ]]; then ok=1; fi
        ;;
      *)
...
echo
printf '%sRYK ENVELOPE STRESS%s\n' "$BOLD" "$RESET"
printf 'binary: %s\n' "$RYK_BIN"
printf '%sproves ryk hook/evaluate/bare stdin — not live host CLIs%s\n' "$DIM" "$RESET"
hr
...
echo
hr
printf '%sAdversarial extras%s (default-pack hits on cursor + claude envelopes)\n' "$CYAN" "$RESET"
