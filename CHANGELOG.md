...
* **Bare `ryk` empty/whitespace stdin on a non-TTY hook entry is fail-closed.** `src/cli/mod.zig` used to map `agent_hook.command`'s blank stdin (`NotAgentHookInput`) to help + exit 0. Hosts that treat exit 0 / non-JSON as allow skipped the gate. Empty and whitespace-only stdin now emit the same dual-contract deny JSON and exit 2 as other pre-eval failures. Interactive TTY with no args still shows help.
