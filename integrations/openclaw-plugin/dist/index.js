...
function configuredWorkspaceRoot(api) {
...
export const ENFORCEMENT_NOTE = 'supported install is curl-installed ryk + ryk agents setup openclaw; npm/ClawHub paths are sunset; metadata passes are unprotected; prefer wrapper: ryk run -- openclaw';
/** Standing warning text for metadata/discovery passes where api.on is not live. */
export const UNPROTECTED_NOOP_WARNING = `[ryk] unprotected: OpenClaw did not provide a live full-runtime hook registration API, ` +
...
const KNOWN_REGISTRATION_MODES = new Set([
    'full',
    'discovery',
    'tool-discovery',
    'setup-only',
...
export function isUnattended(environ = process.env) {
...
function redactSecrets(data, seen = new WeakSet()) {
    ...
    try {
        if (Array.isArray(data))
            return data.map((value) => redactSecrets(value, seen));
        const result = {};
...
function validateJsonSafe(data, seen = new WeakSet(), depth = 0, budget = { nodes: 0, stringBytes: 0 }) {
    budget.nodes += 1;
...
function encodeHookPayload(event, data, sessionId) {
    let json;
    try {
...
function sanitizeDiagnostic(value) {
    ...
    const opaqueTokenPattern = new RegExp(`\\b(?:gh[pousr]_[A-Za-z0-9_]+|${providerTokenPrefix}[A-Za-z0-9_-]+)\\b`, 'g');
    return raw
 ...
 */
export function findRyk(cwd, platform = process.platform) {
    ...
    if (pathBin && attestRykCandidate(pathBin, cwd, platform))
        return canonicalPath(pathBin);
    ...
    if (process.env.RYK_ALLOW_WORKSPACE_BIN === '1') {
        ...
        for (const p of candidates) {
            if (attestRykCandidate(p, cwd, platform))
...
export function installerProvenanceValid(binaryPath, receiptPath = join(resolve(binaryPath, '..'), '.ryk-provenance')) {
    try {
        ...
        if (lstatSync(receiptPath).isSymbolicLink())
            return false;
        if (statSync(receiptPath).size > 4096)
            return false;
        ...
        for (const line of readFileSync(receiptPath, 'utf8').split(/\r?\n/)) {
            if (!line)
                continue;
            if (!line.includes('=')) {
                if (line !== 'ryk-provenance-v1' || fields.size !== 0)
                    return false;
                continue;
            }
            ...
            if (!value || (key !== 'path' && key !== 'sha256') || fields.has(key))
                return false;
            fields.set(key, value);
        }
        if (fields.size !== 2 || fields.get('path') !== canonical)
            return false;
...
export function attestRykCandidate(path, cwd, platform = process.platform, allowWorkspaceOverride = process.env.RYK_ALLOW_WORKSPACE_BIN === '1') {
    ...
    if (isUntrustedCandidate(canonical, cwd, allowWorkspaceOverride))
        return false;
    try {
...
function resolveOnPath(name, platform) {
...
export function normalizeOpenClawToolEvent(event) {
...
export function openClawSessionId(ctx) {
...
function failClosedBlock(reason, message, base = {}) {
    return {
        ...base,
        decision: 'block',
        risk: 'high',
...
function softAllow(reason, message) {
    return {
        decision: 'allow',
        risk: 'unknown',
        category: 'unknown',
...
function normalizeBlockingDecision(decision, base, options = {}) {
    if (decision === 'error') {
        return failClosedBlock('ryk_error_response', 'ryk returned an error decision; blocking as a precaution.', { ...base, message: undefined });
    }
    if (decision === 'block') {
        return {
            ...base,
            ...
            reason: base.reason,
            message: sanitizeDiagnostic(base.message) ||
...
export function parseHookResponse(stdout, blocking, options = {}) {
    ...
    if (!stdout.trim()) {
        return fail('ryk_empty_response', 'ryk returned empty output; blocking as a precaution.', 'ryk returned empty output; allowing non-blocking event.');
    }
    let parsed;
    try {
        ...
        return fail('ryk_parse_error', 'ryk returned unreadable JSON; blocking as a precaution.', 'ryk returned unreadable JSON; allowing non-blocking event.');
    }
    if (!parsed || typeof parsed !== 'object') {
        return fail('ryk_missing_decision', 'ryk response missing decision; blocking as a precaution.', 'ryk response missing decision; allowing non-blocking event.');
    }
    const record = parsed;
    ...
    if (decisionRaw === 'block')
        normalized.verifiedPolicyBlock = true;
    return normalized;
}
async function callRyk(rykBin, event, data, sessionId, blocking, logger, options = {}) {
    ...
    if (!attestRykCandidate(rykBin, process.cwd(), process.platform, options.allowWorkspaceBinary)) {
        ...
        return blocking
            ? failClosedBlock('ryk_binary_untrusted', 'ryk executable provenance or identity could not be re-attested; blocking as a precaution.')
    ...
    try {
        ...
        return blocking
            ? failClosedBlock('ryk_hook_error', 'ryk hook failed; blocking as a precaution.')
...
function hasUnknownRegistrationMode(api) {
    const mode = api.registrationMode;
    return mode !== undefined && !KNOWN_REGISTRATION_MODES.has(mode);
}
function registerInertCanaryTool(api) {
    if (typeof api.registerTool !== 'function')
        return false;
    api.registerTool({
        name: INERT_CANARY_TOOL,
        description: 'Inert Ryk health canary. Accepts a synthetic command for policy dispatch proof but never executes it.',
        parameters: {
            type: 'object',
...
export default function rykPlugin(api) {
    ...
    if (typeof api.on !== 'function') {
        logger?.warn?.('[ryk] OpenClaw plugin API does not expose hook registration (api.on). ' +
    ...
    if (onIsNoop) {
        ...
        if (!rykBin) {
            logger?.warn?.('[ryk] Binary not found in PATH (or RYK_BIN). ' +
    ...
    if (!rykBin) {
        if (!registerInertCanaryTool(api)) {
            logger?.warn?.('[ryk] OpenClaw full runtime does not expose api.registerTool; Gateway dispatcher proof is unavailable.');
        }
        logger?.warn?.('[ryk] Binary not found in PATH (or RYK_BIN). ' +
    ...
    const beforeToolCallHandler = async (event, ctx) => {
        ...
        const response = await callRyk(rykBin, 'tool.before', normalized, openClawSessionId(ctx) ?? openClawSessionId(event), true, logger, {
        ...
        if (response.decision === 'block') {
            ...
            const isExactCanary = normalized.toolName === INERT_CANARY_TOOL &&
    ...
    api.on('session_start', async (event, ctx) => {
        ...
        await callRyk(rykBin, 'session.start', { session_id: event?.sessionId }, openClawSessionId(ctx) ?? openClawSessionId(event), false, logger, { allowWorkspaceBinary });
    });
    api.on('after_tool_call', async (event, ctx) => {
        await callRyk(rykBin, 'tool.after', normalizeOpenClawToolEvent(event), openClawSessionId(ctx) ?? openClawSessionId(event), false, logger, { allowWorkspaceBinary });
    });
    api.on('session_end', async (event, ctx) => {
        await callRyk(rykBin, 'session.end', event, openClawSessionId(ctx) ?? openClawSessionId(event), false, logger, { allowWorkspaceBinary });
    });
}
