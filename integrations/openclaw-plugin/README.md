...
| `tool.before` | Before OpenClaw invokes a tool | **Blocking when hooks fire** — empty/malformed fail closed; leftover unused policy ask is rewritten by `ryk hook`; a leaked `ask` is fail-closed deny; legacy and metadata passes are unprotected |
...
OpenClaw’s `before_tool_call` result can evolve across host versions. This adapter only relies on the stable blocking shape. Leftover unused policy ask is rewritten by `ryk hook`. A leaked `ask` is fail-closed deny. Stage, SoftBlock, and FM steward ask never become allow. Blocking is effective **only if** `before_tool_call` runs.
...
- `decision: "ask"` → **block** (unexpected after `ryk hook` rewrite). Leftover unused policy ask is rewritten by `ryk hook` before emit. Host extra `RYK_OPENCLAW_UNATTENDED` folds into `--ci`. There is no OpenClaw ask UI.
- unrecognized → **block**
...
- Blocking hooks fail closed on empty/malformed responses, unrecognized decisions, leaked `ask`, cyclic or BigInt input, and payloads larger than 1 MiB. Leftover unused policy ask is rewritten by `ryk hook`.
