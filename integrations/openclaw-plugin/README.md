...
| `tool.before` | Before OpenClaw invokes a tool | **Blocking when hooks fire** — empty/malformed fail closed; leftover unused `ask` is permit (unattended → block); legacy and metadata passes are unprotected |
...
OpenClaw’s `before_tool_call` result can evolve across host versions. This adapter only relies on the stable blocking shape. Leftover unused policy `ask` is permit on the attended path (same coding-host wire as Grok). Unattended leftover `ask` still blocks. Explicit deny, stage, SoftBlock, and FM steward ask never become allow. Blocking is effective **only if** `before_tool_call` runs.
...
- `decision: "ask"` → **allow** leftover unused policy ask (unattended / `--ci` → **block**). There is no OpenClaw ask UI. Stage / SoftBlock / FM ask are remapped to block by `ryk hook` before emit.
- unrecognized → **block**
...
- Blocking hooks fail closed on empty/malformed responses, unrecognized decisions, cyclic or BigInt input, and payloads larger than 1 MiB. Attended leftover unused `ask` is permit; unattended leftover `ask` blocks.
